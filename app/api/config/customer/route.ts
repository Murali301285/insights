...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (body.slno) {
            ...
            const created = await prisma.customerMaster.create({
                data: {
                    ...data,
        ...
        console.error(error)
        if (error.code === 'P2002') return NextResponse.json({ error: "Customer Name must be unique" }, { status: 400 });
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        const deleted = await prisma.customerMaster.update({
