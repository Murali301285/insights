...
export async function GET(request: NextRequest) {
    try {
        ...
        const data = await prisma.customerCategoryMaster.findMany({
            where: {
                isDelete: false,
...
export async function POST(request: NextRequest) {
    ...
    try {
        ...
        if (body.slno) {
            const updated = await prisma.customerCategoryMaster.update({
            ...
            const created = await prisma.customerCategoryMaster.create({
                data: {
                    ...data,
        ...
        console.error(error);
        if (error.code === 'P2002') return NextResponse.json({ error: "Category name must be unique" }, { status: 400 });
...
export async function DELETE(request: NextRequest) {
    ...
    try {
        ...
        const deleted = await prisma.customerCategoryMaster.update({
