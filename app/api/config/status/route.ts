...
export async function POST(req: Request) {
    try {
        ...
        if (body.slno) {
            const updated = await prisma.statusMaster.update({
            ...
            const created = await prisma.statusMaster.create({
                data: {
                    statusName: body.statusName,
                    order: orderInt,
                    remarks: body.remarks,
        ...
        console.error("Status Master Error:", error);
        if (error.code === 'P2002') {
            return NextResponse.json({ error: "A status with this Name or Order number already exists." }, { status: 400 });
        }
...
export async function DELETE(req: Request) {
    try {
        ...
        await prisma.statusMaster.update({
