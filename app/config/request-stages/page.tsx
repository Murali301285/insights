...
export default function RequestStagesMasterPage() {
    const { setHeaderInfo } = useHeader()
    const [data, setData] = useState<RequestStageEntry[]>([])
    const [loading, setLoading] = useState(true)
    const [companies, setCompanies] = useState<any[]>([])
    const [selectedCompanyId, setSelectedCompanyId] = useState<string>("all")
    const [isAddOpen, setIsAddOpen] = useState(false)
    const [editItem, setEditItem] = useState<RequestStageEntry | null>(null)

    useEffect(() => {
        setHeaderInfo("Request Stages", "Configure sequential stages for support & request pipelines.")
    ...
    async function fetchData() {
        ...
        try {
            ...
            const res = await fetch(`/api/config/request-stages${qs}`)
            if (res.ok) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const res = await fetch("/api/config/request-stages", {
                method: "POST",
    ...
    async function handleDelete(slno: number) {
        ...
        try {
            const res = await fetch(`/api/config/request-stages?slno=${slno}`, { method: "DELETE" })
            if (res.ok) {
                ...
                <Dialog open={isAddOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div className="space-y-2">
                                <Label htmlFor="statusName">Stage Name</Label>
                            ...
                            <div className="space-y-2">
                                ...
                                <p className="text-xs text-zinc-500">Determines the flow direction (e.g. 1, 2, 3).</p>
                            </div>
                            <div className="space-y-2">
