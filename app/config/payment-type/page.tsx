...
export default function PaymentTypeMasterPage() {
    ...
    useEffect(() => {
        setHeaderInfo("Payment Type Master", "Configure and manage payment options.")
    ...
    async function fetchData() {
        ...
        try {
            ...
            const res = await fetch(`/api/config/payment-type${qs}`)
            if (res.ok) {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            const res = await fetch("/api/config/payment-type", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify(payload)
            })
            if (res.ok) {
                toast.success(editItem ? "Payment Type updated" : "Payment Type created")
                setIsAddOpen(false)
                setEditItem(null)
                fetchData()
            } else {
    ...
    async function handleDelete(slno: number) {
        ...
        try {
            const res = await fetch(`/api/config/payment-type?slno=${slno}`, { method: "DELETE" })
            if (res.ok) {
                ...
                return (
                    <HoverCard>
                        <HoverCardTrigger asChild>
                            <span className="cursor-pointer font-medium hover:underline text-emerald-700">{item.paymentType}</span>
                        </HoverCardTrigger>
                        <HoverCardContent className="w-64" side="right">
                            <div className="space-y-2">
                                <h4 className="text-sm font-semibold">{item.paymentType}</h4>
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-zinc-200 shadow-sm">
                ...
                <Dialog open={isAddOpen} onOpenChange={(open) => {
                    ...
                    <DialogContent>
                        <DialogHeader>
                            <DialogTitle>{editItem ? "Edit Payment Type" : "Add New Payment Type"}</DialogTitle>
                            <DialogDescription>
                                Enter the details.
                            </DialogDescription>
                        </DialogHeader>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <DialogFooter>
                                <Button type="submit">{editItem ? "Update" : "Create"} Payment Type</Button>
                            </DialogFooter>
                        </form>
                    </DialogContent>
                </Dialog>
