            ...
            <div className="grid grid-cols-12 gap-6 h-auto">
                <div className="col-span-12 lg:col-span-9 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {/* Funnel Quantities */}
                        <div className="bg-white rounded-2xl p-6 border border-zinc-200/60 shadow-sm relative overflow-hidden transition-all hover:shadow-md h-full flex flex-col">
                            <div className="flex justify-between items-start mb-1">
                                <div className="flex items-center gap-2">
                                    <ShoppingCart className="w-5 h-5 text-emerald-600" />
                                    <h3 className="font-bold text-zinc-900 text-lg">Orders (in Nos)</h3>
                                </div>
                                <button onClick={() => setOrdersNosModalOpen(true)} className="text-zinc-400 hover:text-emerald-600 transition-colors z-20">
                                    <Eye className="w-4 h-4" />
                                </button>
                            </div>
                            <p className="text-zinc-500 text-sm mb-6">Lead to Order conversion</p>
                            ...
                            <div className="flex gap-2 text-sm text-zinc-500 w-full overflow-hidden mt-auto">
                        ...
                        <div className="bg-white rounded-2xl p-6 border border-zinc-200/60 shadow-sm relative overflow-hidden transition-all hover:shadow-md h-full flex flex-col">
                            <div className="flex justify-between items-start mb-1">
                                ...
                                <button onClick={() => setOrdersValueModalOpen(true)} className="text-zinc-400 hover:text-purple-600 transition-colors z-20">
                                    <Eye className="w-4 h-4" />
                                </button>
                            </div>
                            <p className="text-zinc-500 text-sm mb-6">Total Pipeline Context</p>

                            <div className="flex items-center gap-3 mb-4 flex-1">
                                <span className="text-4xl font-extrabold text-zinc-900">
                                    {formatCurrency((latest.quotesValue || 0) + (latest.negotiationValue || 0) + (latest.winValue || 0), currency)}
                                </span>
                            </div>
                            <div className="flex gap-2 text-sm font-semibold w-full overflow-hidden mt-auto">
                                <span className="text-emerald-600 shadow-sm px-2 py-0.5 rounded-md">Wins: {formatCurrency(latest.winValue || 0, currency)}</span>
                                <span className="text-rose-600 shadow-sm px-2 py-0.5 rounded-md">Losses: {formatCurrency(latest.lossValue || 0, currency)}</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-white rounded-2xl p-6 border border-zinc-200/60 shadow-sm relative overflow-hidden min-h-[350px] flex flex-col justify-between hover:shadow-md transition-shadow">
                            ...
                            <div className="flex items-center gap-3 z-20">
                                <div className="flex bg-zinc-50 p-1 rounded-md border border-zinc-200/60">
                                    <button
                                        ...
                                        className={cn("px-4 py-1.5 text-xs rounded font-semibold transition-all", funnelMode === "Nos" ? "bg-white text-zinc-900 shadow-sm border border-zinc-200/40" : "text-zinc-500 hover:text-zinc-800")}
                                    >
                                        Count
                                    </button>
                                    <button
                                        onClick={() => setFunnelMode("Values")}
                                        className={cn("px-4 py-1.5 text-xs rounded font-semibold transition-all", funnelMode === "Values" ? "bg-white text-zinc-900 shadow-sm border border-zinc-200/40" : "text-zinc-500 hover:text-zinc-800")}
                                    >
                                        Revenue
                                    </button>
                                </div>
                                <button onClick={() => setFunnelDetailsModalOpen(true)} className="p-2 rounded-md border border-zinc-200/60 shadow-sm bg-white hover:bg-zinc-50 text-zinc-500 transition-colors">
                                    <Eye className="w-4 h-4" />
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="w-full h-64 mt-auto">
                            {(() => {
                                const data = funnelMode === "Nos" ? [
                                    { name: "Leads", value: latest.leadsCount || 0, fill: "bg-[#dce9fc]" },
                                    { name: "RFQs", value: latest.rfqCount || 0, fill: "bg-[#8abbfc]" },
                                    { name: "Quotation", value: latest.quotesCount || 0, fill: "bg-[#4da1fe]" },
                                    { name: "Negotiation", value: latest.negotiationCount || 0, fill: "bg-[#1c64eb]" },
                                    { name: "Orders", value: latest.orderCount || 0, fill: "bg-[#0b48b5]" }
                                ] : (() => {
                                    ...
                                    return [
                                        { name: "Leads", value: formatCurrency(estLeadsValue || 0, currency), fill: "bg-indigo-200/80" },
                                        { name: "RFQs", value: formatCurrency(estRfqValue || 0, currency), fill: "bg-indigo-300" },
                                        { name: "Quotation", value: formatCurrency(latest.quotesValue || 0, currency), fill: "bg-indigo-400" },
                                        { name: "Negotiation", value: formatCurrency(latest.negotiationValue || 0, currency), fill: "bg-indigo-500" },
                                        { name: "Orders", value: formatCurrency(latest.winValue || 0, currency), fill: "bg-indigo-600" }
                                    ];
                                })();

                                return (
                                    <div className="w-full h-full flex flex-col overflow-hidden py-1" style={{ clipPath: 'polygon(15% 0, 85% 0, 60% 100%, 40% 100%)' }}>
                                        {data.map((d, i) => (
                                            <div key={i} className={`flex-1 w-full flex items-center justify-center gap-3 border-b-2 border-white last:border-0 hover:opacity-90 transition-opacity ${d.fill}`}>
                                                <span className="font-extrabold text-[15px] drop-shadow-md text-white/95 truncate">{d.name}</span>
                                                <span className="font-black text-[16px] drop-shadow-md text-white truncate">{d.value}</span>
                                            </div>
                                        ))}
                                    </div>
                                );
                ...
                <div className="col-span-12 lg:col-span-3 relative min-h-[500px] lg:min-h-0">
                    <div className="w-full h-full lg:absolute lg:inset-0 bg-white rounded-3xl p-4 flex flex-col border border-zinc-200 shadow-sm overflow-hidden">
                        ...
                        <div className="flex-1 space-y-3 overflow-y-auto relative pr-2 pb-2 custom-scrollbar">
                            {(latest.recentActivities || [])
                                .filter((a: any) => new Date(a.date).getTime() >= Date.now() - 14 * 24 * 60 * 60 * 1000)
                                .slice(0, 10).map((activity: any, i: number) => {
